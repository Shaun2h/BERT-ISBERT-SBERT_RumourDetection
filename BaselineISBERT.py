...
if __name__=="__main__":
    ...
    for eventwrap in indo_traintestindexes+traintestindexes+list(eventrefdict.keys()): # isolate a single event as the TEST case, or run it as a split.
        ...
        for currentepoch in range(max_epoch):
            ...
            correctcount,totalsubjects,totalloss = train_loops.baseline_ISBERT_loop(classifier_model, train_dataloader,loss_fn = loss_fn, optimizer=classifier_optimizer,batch_size=batch_size,device=device,name=modelname,epoch=currentepoch,backprop=True,threshold=decision_threshold)
            ...
            print("Test")
            totalsubjects,totalloss,correctcount = train_loops.baseline_ISBERT_loop(classifier_model,test_dataloader,batch_size=batch_size,device=device,loss_fn=loss_fn,name=modelname,epoch=currentepoch,threshold=decision_threshold)
